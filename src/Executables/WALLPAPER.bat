...
PowerShell -NoP -C "$Content = (Get-Content '%WINDIR%\Resources\Themes\aero.theme'); $Content = $Content -replace 'Wallpaper=%%SystemRoot%%.*', 'Wallpaper=%%SystemRoot%%\web\wallpaper\Windows\Ryne-Space.bmp'; $Content = $Content -replace 'SystemMode=.*', 'SystemMode=Dark'; $Content -replace 'AppMode=.*', 'AppMode=Light' | Set-Content '%WINDIR%\Resources\Themes\aero.theme'"

@echo OFF

if exist "Ryne-Space.bmp" (
	echo move /y "Ryne-Space.bmp" "%WINDIR%\Web\Wallpaper\Windows"
	move /y "Ryne-Space.bmp" "%WINDIR%\Web\Wallpaper\Windows"
	echo icacls "%WINDIR%\Web\Wallpaper\Windows\Ryne-Space.bmp" /reset
	icacls "%WINDIR%\Web\Wallpaper\Windows\Ryne-Space.bmp" /reset
)
...
if not exist "%WINDIR%\Web\Wallpaper\Windows\Ryne-Space.bmp" set "wallFail=true" & goto lockScreen

echo reg add "HKEY_USERS\%~1\Control Panel\Desktop" /v WallPaper /t REG_SZ /d "%WINDIR%\Web\Wallpaper\Windows\Ryne-Space.bmp" /f
reg add "HKEY_USERS\%~1\Control Panel\Desktop" /v WallPaper /t REG_SZ /d "%WINDIR%\Web\Wallpaper\Windows\Ryne-Space.bmp" /f
